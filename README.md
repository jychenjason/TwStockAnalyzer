...
讓 TWSE 少回這種 428 的三道防線（都集中在 `TWSESource._get`）：

- **瀏覽器樣請求外觀**：所有請求走同一支 `requests.Session`，帶瀏覽器 `User-Agent`／`Accept`／`Referer`，批次開跑前先 GET 網站根領 WAF cookie——被 WAF 判定為自動化（因而一律 428）的請求，等多久重試都不會過，只有改請求外觀才過得了。
- **請求間距與全域冷卻**：每筆請求之間至少隔 `HTTP_MIN_INTERVAL`（0.5 秒）；收到 428/429 後冷卻以 `COOLDOWN_STEP`（5 秒）逐次加倍（封頂 `COOLDOWN_CAP` 60 秒），冷卻是 **source 級**的——一檔觸發，下一檔也一起等，批次不再一檔接一檔去續命限流窗。
- **風暴長退避**：連續 3 次暫時性錯誤後改用 30 秒長退避，不再短連發；`Retry-After` header 永遠優先於以上三者。
